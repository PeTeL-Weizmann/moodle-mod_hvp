...
ns.Html.prototype.getCKEditorConfig = function () {
  ...
  config.plugins.push(DirectionButtonsPlugin);
  config.toolbar.push('textDirection-rtl', 'textDirection-ltr');
...
/**
 * CKEditor5 plugin for adding RTL and LTR text direction buttons in H5P editors.
 *
 * The buttons apply the `dir` attribute (either "rtl" or "ltr") to selected blocks.
 *
 * Usage:
 *   config.plugins.push(DirectionButtonsPlugin);
 *   config.toolbar.push('textDirection-rtl', 'textDirection-ltr');
 *
 * @class DirectionButtonsPlugin
...
class DirectionButtonsPlugin {
  ...
  init() {
    ...
    // Enable conversion for `dir` attribute
    editor.conversion.for('upcast').attributeToAttribute({ view: 'dir', model: 'dir' });
    editor.conversion.for('downcast').attributeToAttribute({ model: 'dir', view: 'dir' });

    const ButtonView = editor.ui.componentFactory.create('bold').constructor;

    const directions = ['rtl', 'ltr'];

    // Hardcoded today's date for cache busting
    const cachebuster = '20240423';

    directions.forEach(dir => {
      editor.ui.componentFactory.add(`textDirection-${dir}`, locale => {
        const button = new ButtonView(locale);

        const tooltip = ns.t('core', `directionButton${dir.toUpperCase()}`) || `Align text ${dir === 'rtl' ? 'right-to-left' : 'left-to-right'}`;
        ...
        // Inject SVG icon on render
        button.on('render', () => {
          if (button.element) {
            const contextRoot = window.location.pathname.split('/')[1];
            const moodleRoot = `/${contextRoot}`;
            const img = document.createElement('img');
            img.src = `${moodleRoot}/mod/hvp/pix/dir_${dir.toUpperCase()}.svg?${cachebuster}`;
            img.alt = tooltip;
            ...
            button.element.appendChild(img);
          }
        });

        // Apply the `dir` attribute when clicked
        button.on('execute', () => {
          editor.model.change(writer => {
            ...
            for (const item of range.getItems()) {
              ...
              writer.setAttribute('dir', dir, parent);
            }
          });
        });
